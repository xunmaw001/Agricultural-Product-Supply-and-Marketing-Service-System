    ...
    <fieldset class="search-container" style="text-align: center;margin: 50px 0;">
      <form class="layui-form" style="margin: 10px 0;">
        ...
        <div class="layui-inline" style="margin-left: 30px;">
          ...
          <button v-if="isAuth('nongchanpin','新增')" @click="jump('../nongchanpin/add.jsp')" type="button" class="layui-btn btn-theme">
      ...
      <div class="box">
        <ul class="clearfix">
          <li v-for="(item,index) in dataList" v-bind:key="index">
            <a :href="'../nongchanpin/detail.jsp?id='+item.id">
              <figure>
                <img :src="item.chanpintupian?item.chanpintupian.split(',')[0]:''" width="410" height="272" />
              </figure>
              <h2>{{item.chanpinmingcheng}}</h2>
              <div class="div1">
                <b></b>
  ...
  <script>
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage', 'http', 'jquery'], function() {
      ...
      function pageList() {
        ...
        if (jquery('#chanpinbianhao').val()) {
          param['chanpinbianhao'] = jquery('#chanpinbianhao').val() ? '%' + jquery('#chanpinbianhao').val() + '%' : '';
        }  
        if (jquery('#chanpinmingcheng').val()) {
          param['chanpinmingcheng'] = jquery('#chanpinmingcheng').val() ? '%' + jquery('#chanpinmingcheng').val() + '%' : '';
        }  
        if (jquery('#laizidiqu').val()) {
          param['laizidiqu'] = jquery('#laizidiqu').val() ? '%' + jquery('#laizidiqu').val() + '%' : '';
        }  
        // 获取列表数据
        http.request('nongchanpin/list', 'get', param, function(res) {
